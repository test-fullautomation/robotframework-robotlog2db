...
\item[]
\item
  Project/Variant (can be overwritten by argument \rlog{--variant} or
  \rlog{--config} of
...
\end{robotcode}
\item
  Component (can be overwritten by argument \rlog{--config} of
...
\tightlist
\item
  \rlog{Project}: will be set to default value \pcode{ROBFW} if not defined.
\item
  \rlog{Software version}: will be set to execution time
  \pcode{\%Y\%m\%d\_\%H\%M\%S} as default value.
\item
  \rlog{Component}: will be set to default value \pcode{unknown} if not
  defined.
\end{itemize}
...
\begin{itemize}
\item
\rlog{--variant VARIANT}

  To specify the {Project/Variant} information.
\item
\rlog{--versions VERSIONS}
